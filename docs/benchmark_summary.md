...
For the latest **gradient-only** comparison, see the
[frozen worker-scaling study](rk_worker_scaling_benchmark.md): six seeds per
dimension in 5D–7D, 1/2/4/8 workers, reversed confirmation, and repeated
regression controls. RK usually beats PLS, but a repeatable 7D exception and
uneven worker task lifetimes rule out a universal fastest-algorithm claim.
Construction remains separate; no algorithm change or TTK rerun.

For the latest RK optimization, see the
...
For the preceding RK optimization and RK/PLS/F-Max comparison
...
see the [simplicial gradient study](simplicial_gradient_benchmark.md).
It reports the PLS workspace optimization, critical counts, timing boundaries
and higher-dimensional cases where RK does not establish a speed advantage.
The persistence tables below remain separate historical experiments.
...
Raw CSVs, manuscript prose fragments, manuscript PDFs, and private discussion
notes are intentionally kept out of the public package repository.
